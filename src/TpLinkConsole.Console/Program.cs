...
// }
var router = new TPLinkRouter("admin", "admin", new Uri("http://192.168.0.1"), new ScriptVariableExtractor(new Tokenizer()), new HtmlScriptExtractor(), new HttpClient());
Console.WriteLine("Refreshing router");
await router.RebootAsync();
Console.WriteLine("Refreshed successfully");